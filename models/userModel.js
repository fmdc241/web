...
const getUserById = async (id) => {
  const result = await pool.query(
    'SELECT id, username, full_name, email, address, is_admin FROM users WHERE id = $1', 
    [id]
  );
  return result.rows[0];
};
...
const getAllUsers = async () => {
  const result = await pool.query(
    'SELECT id, username, full_name, email, address, is_admin, created_at FROM users WHERE is_admin = false'
  );
  return result.rows;
};
